<!--
Enhancements that remaining
1. Only pull in new grid recommendations with actual empty locations
2. Need to determine if it goes to Mezz or SMPM
...
<html>
    ...
    <head>
        <title>Change Log Media Settings</title>
        <?php
    ...
    <body style="background-color: #FFFFFF;">
        <div class="main padder" style="margin-top: 75px;padding-left: 100px; "> 
            ...
            ?>
            <h2 style="padding-bottom: 0px;">Ok in Storage Media Change Log</h2>
...
#Query the Database into a result set - 
                        $result = $aseriesconn->prepare("SELECT ATWHSE as Whse,  ATITEM as Item, ATFNAM as Field, ATBVAL as before, ATAVAL as after, ATCHGU as TSM, date('20' || substr(ATCHGD,2,2) || '-' || substr(ATCHGD,4,2) || '-' || substr(ATCHGD,6,2)) as Date, LOLOC# as LOCATION FROM A.HSIPCORDTA.NPFCAT,  A.HSIPCORDTA.NPFLOC WHERE ATFNAM in ('PCTOTE','PCSHLF') and ATAVAL = 'N' and LOWHSE = '" . $var_whse . "' and LOSCDE = 'A' and LOITEM = ATITEM and date('20' || substr(ATCHGD,2,2) || '-' || substr(ATCHGD,4,2) || '-' || substr(ATCHGD,6,2)) = '" . $var_datepick . "'");
                        $result->execute();
                        ...
                        foreach ($resultsetarray as $key => $value) {
                            ...
                            $var_LOLOC =$resultsetarray[$key][7];
