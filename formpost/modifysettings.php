...
$recid = $conn1->prepare("SELECT SETTINGS_ID from slotting.item_settings WHERE WHSE =  $var_whse and ITEM = $var_item and PKGU =  $var_pkgu");
$recid->execute();
...
if (isset($id)) {
    ...
    $sql = "UPDATE slotting.item_settings SET $varset WHERE SETTINGS_ID = $id;";
    $query = $conn1->prepare($sql);
    $query->execute();
} else {
    //no record exists, create new record
    $sql = "INSERT INTO slotting.item_settings (SETTINGS_ID, WHSE, ITEM, PKGU, PKGU_TYPE, CASETF,  HOLDTIER, HOLDGRID, HOLDLOCATION) VALUES (0, $var_whse,  $var_item,  $var_pkgu, '$var_pkgutype', '$var_casetf', '$var_holdtier', '$var_holdgrid', '$var_holdloc');";
    $query = $conn1->prepare($sql);
    $query->execute();
}
