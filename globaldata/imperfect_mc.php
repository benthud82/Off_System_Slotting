...
foreach ($acceptablebays as $value_l2) {
    ...
    $IMPERFMC = $IMPERF_MC_TIER . $IMPERF_MC_GRID5 . $IMPERF_MC_DEEP . $value_l2;
    ...
    if ($IMPERFECT_MC_key <> FALSE) { //a perfect grid match has been found.  Set as new location
        $IMPERFECT_MC_NEW_LOC = $EMPTYLOC_array[$IMPERFECT_MC_key]['LMLOC#'];
        $displayarray[$topcostkey]['IMPERFECT_MC_SLOT_LOC'] = $IMPERFECT_MC_NEW_LOC;
        $IMPERFECT_MC_NEW_GRD5 = $EMPTYLOC_array[$IMPERFECT_MC_key]['LMGRD5'];
        $displayarray[$topcostkey]['AssgnGrid5'] = $IMPERFECT_MC_NEW_GRD5; //Add new grid5 to display array
        $NEW_GRD_HGT = $EMPTYLOC_array[$IMPERFECT_MC_key]['LMHIGH'];
        $NEW_GRD_DPT = $EMPTYLOC_array[$IMPERFECT_MC_key]['LMDEEP'];
        $NEW_GRD_WDT = $EMPTYLOC_array[$IMPERFECT_MC_key]['LMWIDE'];
        $imperfectbay = substr($IMPERFECT_MC_NEW_LOC, 3, 2);
...
// START - New Logic
        ...
        if ($zone == 'CSE') { //calculate LSE or CSE walk cost
            ...
            $walk_score_imperfectMC = _walkscore(intval(substr($IMPERFECT_MC_NEW_LOC, 3, 2)), $OPT_OPTBAY, $TOP_REPLEN_COST_array[$topcostkey]['AVG_DAILY_PICK']);
        }
