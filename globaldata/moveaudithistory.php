...
if ($var_whse > 10) {
    ...
    //detail data query
$result1 = $aseriesconn_can->prepare("SELECT MVTITM, MVTYPE, MVFLC#, MVTLC#, date(substr(MVCNFD,1,4) || '-' || substr(MVCNFD,5,2) || '-' || substr(MVCNFD,7,2)) as DATE,  MVREQT, MVREQQ, MVCNFQ FROM A.ARCPCORDTA.NPFMVE WHERE (MVTPKG <> 0) and MVCNFQ<>0 and (MVDESC like 'COMPLETED%' or MVDESC like 'MAN%') and MVCNFD >= $startdate and MVTITM = '$var_item' and MVWHSE = $var_whse  $zonesql GROUP BY MVTITM, MVTYPE, MVFLC#, MVTLC#, date(substr(MVCNFD,1,4) || '-' || substr(MVCNFD,5,2) || '-' || substr(MVCNFD,7,2)),  MVREQT, MVREQQ, MVCNFQ ORDER BY  date(substr(MVCNFD,1,4) || '-' || substr(MVCNFD,5,2) || '-' || substr(MVCNFD,7,2)) DESC");
$result1->execute();
$result1array = $result1->fetchAll(pdo::FETCH_ASSOC);

//summary data query
$result2 = $aseriesconn_can->prepare("SELECT sum(case WHEN MVTYPE = 'RS' then 1 else 0 end) as AUTOS, sum(case WHEN MVTYPE = 'SK' or MVTYPE = 'SJ' or MVTYPE = 'SP' or MVTYPE = 'SO' then 1 else 0 end) as ASOS,  sum(case WHEN MVTYPE = 'CM' then 1 else 0 end) as CONSOLS from A.ARCPCORDTA.NPFMVE WHERE (MVTPKG <> 0) and MVCNFQ<>0 and (MVDESC like 'COMPLETED%' or MVDESC like 'MAN%') and MVCNFD >= $startdate and MVTITM = '$var_item' and MVWHSE = $var_whse  $zonesql ");
$result2->execute();
