...
$casepkgusql = $conn1->prepare("SELECT DISTINCT
                                                            ...
                                                            FROM
                                                                slotting.ipopps
                                                                    LEFT JOIN
                                                                slotting.caseipreview ON caseip_item = VCITEM
                                                                    AND caseip_whse = VCWHSE
                                                                    LEFT JOIN
                                                                slotting.slotting_itemcomments ON VCWHSE = itemcomments_whse
                                                                    AND itemcomments_item = VCITEM
                                                            WHERE
                                                                VCWHSE = $var_whse and (caseip_iporcase IS NULL or caseip_iporcase =  'INNER')  $includesql; ");
$casepkgusql->execute();
...
foreach ($casepkgusqlarray as $key => $value) {
    ...
    $IPPKGU = $casepkgusqlarray[$key]['IPPKGU'];
