...
$var_building = $_POST['building'];
if ($var_whse == 3 && $var_building == 2) {
    $sparkssql = " and (BAY >= 'W4000' or BAY like 'NON%')";
    $sparkstextsql = " and TEXT >= 'W40'";
} elseif ($var_whse == 3 && $var_building == 1) {
    $sparkssql = " and (BAY < 'W4000' or BAY like 'NOT%') and BAY not like 'NON%'";
    $sparkstextsql = " and TEXT < 'W40'";
} else {
    $sparkssql = " ";
    $sparkstextsql = " ";
...
if ($var_zone == 'L%') {
    ...
    $baycolor = $conn1->prepare("SELECT 
                                ...
                                slotting.vectormap
                                    LEFT JOIN
                                slotting.optimalbay_hist ON optbayhist_whse = VECTWHSE
                                    and optbayhist_bay = BAY
                                WHERE
                                    VECTWHSE = $var_whse and CSLS = 'LSE' 
...
if ($var_zone == 'L%') {
    $loosetext = $conn1->prepare("SELECT 
                            ...
                            FROM
                                slotting.loosetext
                            WHERE
                                WHSE = $var_whse  and TEXT not like '%Replens%'");  //$orderby pulled from: include 'slopecat_switch_orderby.php';
    $loosetext->execute();
