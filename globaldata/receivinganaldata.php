...
$whslocprim = $conn1->prepare("SELECT S.*, AVG_DAILY_UNIT FROM slotting.whse_status S  JOIN slotting.my_npfmvc on WAREHOUSE = LOWHSE and ITEM_NUMBER = LOITEM and PACKAGE_UNIT = LOPKGU WHERE LOWHSE = $var_whse and LOITEM = $itemnum and LOMAXC > 0 order by LOPKGU asc");
$whslocprim->execute();
$whslocprimarray = $whslocprim->fetchAll(pdo::FETCH_ASSOC);

$whslocres = $conn1->prepare("SELECT * FROM slotting.whse_status WHERE LOWHSE = $var_whse and LOITEM = $itemnum and LOMAXC = 0 order by LOLOC asc");
$whslocres->execute();
